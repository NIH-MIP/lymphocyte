...
######to set up, change CNNModel and head_map_out directories####
...
parser.add_argument('--outfile')  # where you want model saved
parser.add_argument('--out_dir')  # where you want file saved
args = parser.parse_args()


TileFolder = args.image_dir
print(TileFolder)
LearningRate = theano.shared(np.array(5e-3, dtype=np.float32));
BatchSize = 96;

CNNModel = '/data/MIP/code/u24_lymphocyte-develop/cnn_lym_model.pkl';
heat_map_out = args.out_dir;#'/data/MIP/TCGABLCA/TIL/heat_map/';

mu = 0.6151888371;
sigma = 0.2506813109;
aug_fea_n = 1;

svs_pw_glob = 100;  # power of SVS file
png_pw_glob = 100;  # power of png file
...
def load_data(todo_list, rind):
    ...
    for fn in todo_list:
        lind += 1;
        full_fn = os.path.join(TileFolder,fn);
        ...
        x_off = float(fn.split('_')[2].split('-')[1]);
        y_off = float(fn.split('_')[2].split('-')[2]);
        svs_pw = svs_pw_glob; #can change this up top
        png_pw = png_pw_glob; #power of png file
...
def val_fn_epoch_on_disk(classn, val_fn):
    ...
    return all_or, all_inds, all_coor, todo_list;
...
def split_validation(classn):
    ...
    # Testing
    Or, inds, coor,todo_list = val_fn_epoch_on_disk(classn, val_fn);
    Or_all = np.zeros(shape=(coor.shape[0],), dtype=np.float32);
    Or_all[inds] = Or[:, 0];



    filenames=os.listdir(os.path.join(TileFolder))
    n_img=(png_pw_glob/PS)**2


    fid = open(os.path.join(heat_map_out,args.outfile+'.txt'), 'w')
    filename_index=0
    for name in filenames:
        #print(name)
        name_image=range(filename_index*n_img,((filename_index*n_img)+n_img))
        for i in name_image:
            #print(i)
            x_coord=coor[i][0]
            y_coord = coor[i][1]
            ...
            if filename_index==0:
                ...
                im_index=i%(n_img*filename_index)
            fid.write('{} {} {} {} {}\n'.format(name, x_coord, y_coord, im_index, pred))
        filename_index += 1
    fid.close();
